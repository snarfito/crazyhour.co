...
export default async function PedidosPage() {
  ...
  return (
    <div>
      <h1 className="font-heading text-2xl font-extrabold">Pedidos</h1>
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-lg border border-border p-4">
          <p className="text-sm text-muted-foreground">Pedidos hoy</p>
          <p className="font-heading text-2xl font-extrabold">{ordersToday}</p>
        </div>
        <div className="rounded-lg border border-border p-4">
          <p className="text-sm text-muted-foreground">Ingresos hoy</p>
          <p className="font-heading text-2xl font-extrabold">{formatCOP(revenueToday)}</p>
        </div>
        <div className="rounded-lg border border-border p-4">
          <p className="text-sm text-muted-foreground">Por confirmar</p>
          <p className="font-heading text-2xl font-extrabold">{pendingCount}</p>
        </div>
      </div>
      <Table className="mt-6">
        ...
        <TableBody>
          {(orders ?? []).map((o) => (
            <TableRow key={o.id}>
              <TableCell>{new Date(o.created_at).toLocaleString("es-CO")}</TableCell>
              <TableCell>
