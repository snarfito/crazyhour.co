...
describe("globals.css compiles the expected utilities", () => {
  it("emits font-heading, font-body, font-accent, all brand-color utilities, and the stagger animation", async () => {
    ...
    expect(output).toContain(".font-accent");
    ...
    expect(output).toContain(".animate-stagger-in");
  });
});
