...
describe("verifyWebhookSignature", () => {
  ...
  // Matches Wompi's real transaction.updated payload: `properties` signs
  // [id, status, amount_in_cents] only — reference is not part of the
  // checksum by default (confirmed against Wompi's docs).
  ...
  it("rejects a payload whose `signature.properties` omits a required field", async () => {
    const { verifyWebhookSignature } = await import("./wompi");
    const payload = buildValidPayload();
    // Simulates an attacker stripping `amount_in_cents` out of what's hashed
    // so a tampered amount wouldn't invalidate the checksum.
    payload.signature.properties = ["transaction.id", "transaction.status"];
